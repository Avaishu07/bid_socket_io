...
//       connectWebSocket();
//     }
//     // Fetch live cars only once after initial connect (when connection first becomes stable)
//     if (
//       isConnected &&
//       connectionStatus === 'connected' &&
//       !hasRequestedLiveCarsRef.current
//     ) {
//       hasRequestedLiveCarsRef.current = true;
//       console.log('📤 HomeScreen requesting liveCars once after connect');
//       getLiveCars();
//     }
//     // Reset guard if disconnected so we can fetch again after reconnect
//     if (connectionStatus === 'disconnected') {
//       hasRequestedLiveCarsRef.current = false;
//     }
...
//       }
//     });
//     // intentionally only depend on filteredLiveCars to avoid rerun loops
...
const HomeScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // NOTE: The WebSocket provider now emits `getLiveCars` once on initial connect.
    // Removing the manual getLiveCars() call here prevents duplicate emissions
    // and avoids the re-render / effect loop caused by toggling connection state.
  }, [connectionStatus]);
  // }, []);
